...
- Read-only mode uses Claude Code's `--restricted` (2.1.248+): shell and code tools removed, settings files ignored, file tools confined to the workspace; `--allow` keeps restricted mode and names only the read tools plus the allowed one (older CLIs use `--setting-sources ""`).
