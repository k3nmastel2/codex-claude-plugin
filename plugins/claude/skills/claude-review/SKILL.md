---
name: claude-review
description: Ask Claude Code for a read-only, structured code review of the current git working tree or branch, optionally adversarial. Use when the user says "have Claude review", "get Claude to check my diff", "second set of eyes", "tear this apart", or wants Claude to critique local changes before shipping.
---

# Claude Review

You are a forwarder. Build exactly one companion command, run it, and return its stdout verbatim as your entire response.

## Command

```
node "<plugin-root>/scripts/claude-companion.mjs" review [flags] -- "<focus text>"
```

`<plugin-root>` is two directories above this SKILL.md, i.e. `<this skill's directory>/../../scripts/claude-companion.mjs`. Resolve it to an absolute path before running. Omit `-- "<focus text>"` when the user gave no focus.

## Flag mapping

Flags go before `--`, each as its own argument.

- No flags: reviews the working tree if it is dirty, otherwise the current branch against the detected base branch.
- `--base <ref>` to compare the branch against a specific ref.
- `--scope working-tree` or `--scope branch` to force a mode.
- `--adversarial` when the user wants the change challenged, stress-tested, or "torn apart".
- `--background` for large diffs or when the user asks; otherwise foreground with a shell timeout of at least 1,500,000 ms.

The review always runs read-only. There is no flag that lets it edit anything.

## Rules

- Your entire response is the command's stdout, verbatim, findings in the order the companion prints them. Do not inspect the repository yourself, re-assess or filter findings, or add a preface or postscript.
- After presenting the findings, STOP. Never apply fixes, and do not offer to fix anything until the user chooses which findings to address.
- If the output says the directory is not a git repository, has no commits, or no base branch was found, show it and ask the user for a `--base <ref>` or a commit.
- If the output says Claude is not installed or not logged in, it already names the fix; show it and stop.

## Sandbox

The companion needs network access (Claude's API) and the user's Claude login. If Codex is running commands inside its sandbox, request escalation and run the companion outside the sandbox; do not retry it sandboxed. Output mentioning `CODEX_SANDBOX_NETWORK_DISABLED` means that is exactly what happened.
